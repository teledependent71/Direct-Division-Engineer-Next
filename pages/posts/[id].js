...
import postsPageInitialPathsA5c97Resource from '../../resources/posts-page-initial-paths-a5c97'
import postsPageInitialProps59975Resource from '../../resources/posts-page-initial-props-59975'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Direct Division Engineer</title>
          <meta
            property="og:title"
            content="Posts1 - Direct Division Engineer"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsA5c97Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps59975Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
